...
from .metrics import evaluate_at_length, compute_attention_entropy_detailed
...
def run_full_evaluation(
    ...
    for eval_len in config.eval_seq_lens:
        ...
        entropy_data = compute_attention_entropy_detailed(
            model, dataloaders[key], device, use_amp, max_batches=10
        )
        ...
        results["metrics"][str(eval_len)] = {
            ...
            "attention_entropy_mean": entropy_data["mean"],
            "attention_entropy_per_layer": entropy_data["per_layer"],
        }

        print(
            f"    [{label:>14s}] loss={val_loss:.4f}, "
            f"ppl={val_ppl:.2f}, entropy={entropy_data['mean']:.4f}"
        )
